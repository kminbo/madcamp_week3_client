...
import home_background from '../assets/images/home_background.png';
import { useState } from 'react';
import { generateAIResponse } from '../api/aiApi';
...
const Schopenhauer = () => {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const aiData = { userMessage: input };
            const response = await generateAIResponse('schopenhauer', aiData);

            if (response && response.aiResponse) {
                const schopenhauerReply = { sender: 'schopenhauer', text: response.aiResponse };
                setMessages(prev => [...prev, schopenhauerReply]);
            } else {
            ...
            const errorReply = {
                sender: 'schopenhauer',
                text: '죄송합니다. 일시적인 오류가 발생했어요. 다시 시도해 주세요.',
            };
